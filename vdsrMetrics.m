...
for scaleFactor = scaleFactors
    ...
    for idx = 1:numel(testImages.Files)
        ...
        % PSNR
        bicubicPSNR(idx) = psnr(Ibicubic(:,:,1),Ireference(:,:,1)); %#ok<*AGROW>
        vdsrPSNR(idx) = psnr(Ivdsr(:,:,1),Ireference(:,:,1));
        
        % SSIM
        bicubicSSIM(idx) = ssim(Ibicubic(:,:,1),Ireference(:,:,1));
        vdsrSSIM(idx) = ssim(Ivdsr(:,:,1),Ireference(:,:,1));
